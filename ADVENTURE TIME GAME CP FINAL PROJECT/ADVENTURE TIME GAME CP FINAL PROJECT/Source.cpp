...
bool check_rest_choice(int rest_choice);
void use_inventory(Player& player, int& Player_Attack);
...
bool check_main_choice(int& main_choice)
{
    if (cin.fail() || main_choice < 1 || main_choice > 3)
    {
        cin.clear();  // Clear error flag
        cin.ignore(numeric_limits<streamsize>::max(), '\n');  // Discard invalid input
...
void Embark_on_quest(Player& player)
{
    ...
    Quest quests_Saiyan[] = {
                       {"The Lost Brother ", 1 , 25 ,"Raditz", "Dragon Ball (1)"},

                       {"The Fallen Prince", 2 , 50 ,"Vegeta", "Dragon Ball (2)"},

                       {"Ginyu the Tyrant", 3 , 75 ,"Captain Ginyu", "Dragon Ball (3)"},

                       {"Pride and Shame", 4 , 100 ,"Freeza","Dragon Ball (4)"},

                       {"Shenron the Wise Dragon", 5 , 200 ,"Shenron","Dragon Ball (5)"}


    };
    Quest quests_Ninja[] = {
                      {"The Widow of the Blood Mist Village", 1 , 25 ,"Mei Mei", "Blood Scroll"},

                      {"Prince of the Hidden Sand Village", 2 , 50 ,"Gaara", "Sand Scroll"},

                      {"Killer-Bee the Rapper", 3 , 75 ,"Killer-Bee", "Lightning Scroll"},

                      {"Pain of the Akatsuki", 4 , 100 ,"Onoki","Beast Scroll"},

                      {"Itachi the Genjutsu Master", 5 , 200 ,"Itachi","Sharingan Scroll"}

    };
    Quest quests_Knight[] = {
                     {"Escorting a noble", 1 , 25 ,"Goblin", "Goblin Bag"},

                     {"Trial of Valor", 2 , 50 ,"JIN", "Electric boots"},

                     {"Rescue Mission", 3 , 75 ,"Voldemort", "Daimond Ring"},

                     {"Beast Hunt", 4 , 100 ,"Ghoul","Ghoul skin"},

                     {"Slay the dragon", 5 , 200 ,"Dragon","Dragon Scale"}
...
bool check_attack_choice(int& attack_choice)
{
    if (cin.fail() || attack_choice < 1 || attack_choice > 4)
    {
        cin.clear();  // Clear error flag
        cin.ignore(numeric_limits<streamsize>::max(), '\n');  // Discard invalid input
        cout << "\nInvalid input. Please enter a number between 1 and 3." << endl;
...
void Handle_Attack_Choice(int attack_choice, int& Player_Attack, Player& player)
{
    ...
    case 4:
        ...
        if (count > 0)
        {
            ...
            use_inventory(player, Player_Attack);
        }
        else
        {
            cout << "\n\nInventory is Empty";
            cout << "\n\nYou Did a Random Attack";
            Player_Attack = rand() % 50 + 1;
        }
...
void Rest(Player& player)
{
    ...
    cout << "\n\n-------------Rest Choice------------\n\n";
    cout << "\n\n1.Rest for 20 sec to recover full health";
    cout << "\n\n2.Rest for 5 sec to recover 20% health";

    do {
        cout << "\n\nEnter your Choice: ";
        cin >> rest_choice;
    } while (!check_rest_choice(rest_choice));
    ...
    case 2:
        ...
        player.health += (20 * player.Max_health) / 100;
...
bool check_rest_choice(int rest_choice)
{
    if (cin.fail() || (rest_choice != 1 && rest_choice != 2))
    {
        cin.clear();
...
void use_inventory(Player& player, int& Player_Attack)
{
    ...
    if (player.Class == "Saiyan")
    {
        if (user_choice == 1 && player.inventory[0] != "")
        {
            cout << "\n\n----------State Your Wish-----------\n\n";
            cout << "1.Increase Attack Power for this turn by 25\n2.Recover 25 Health points\n3.Generate a Random Attack from 1-100";
            cout << "\n\nYour Wish: ";
            cin >> wish;
            if (wish == 1)
            {
                Player_Attack = (rand() % (player.level * 50) + 1) + 25;
            }
            else if (wish == 2)
            {
                player.health = +25;
            ...
            {
                Player_Attack = (rand() % 100 + 1);
            }
            player.inventory[0] = "";
        }
        else if (user_choice == 2 && player.inventory[1] != "")
        {
            cout << "\n\n----------State Your Wish-----------\n\n";
            cout << "1.Increase Attack Power for this turn by 50\n2.Recover 50 Health points\n3.Generate a Random Attack from 1-150";
            cout << "\n\nYour Wish: ";
            cin >> wish;
            if (wish == 1)
            {
                Player_Attack = (rand() % (player.level * 50) + 1) + 50;
            }
            else if (wish == 2)
            {
                player.health = +50;
            ...
            {
                Player_Attack = (rand() % 150 + 1);
            }
            player.inventory[1] = "";
        }
        else if (user_choice == 3 && player.inventory[2] != "")
        {
            cout << "\n\n----------State Your Wish-----------\n\n";
            cout << "1.Increase Attack Power for this turn by 75\n2.Recover 75 Health points\n3.Generate a Random Attack from 1-200";
            cout << "\n\nYour Wish: ";
            cin >> wish;
            if (wish == 1)
            {
                Player_Attack = (rand() % (player.level * 50) + 1) + 75;
            }
            else if (wish == 2)
            {
                player.health = +75;
            ...
            {
                Player_Attack = (rand() % 200 + 1);
            }
            player.inventory[2] = "";
        }
        else if (user_choice == 4 && player.inventory[3] != "")
        {
            cout << "\n\n----------State Your Wish-----------\n\n";
            cout << "1.Increase Attack Power for this turn by 100\n2.Recover 100 Health points\n3.Generate a Random Attack from 1-250";
            cout << "\n\nYour Wish: ";
            cin >> wish;
            if (wish == 1)
            {
                Player_Attack = (rand() % (player.level * 50) + 1) + 100;
            }
            else if (wish == 2)
            {
                player.health = +100;
            ...
            {
                Player_Attack = (rand() % 250 + 1);
            }
            player.inventory[3] = "";
        }
        else if (user_choice == 5 && player.inventory[4] != "")
        {
            cout << "\n\n----------State Your Wish-----------\n\n";
            cout << "1.Increase Attack Power for this turn by 150\n2.Recover 150 Health points\n3.Generate a Random Attack from 1-300";
            cout << "\n\nYour Wish: ";
            cin >> wish;
            if (wish == 1)
            {
    ...
    else if (player.Class == "Ninja")
    {
        ...
        {
            cout << "\n\n--------------Blood Scroll-------------\n\n";
            cout << "1.Blood Rush (+25 Damage)\n2.Flowing Red Scale (+25 Health)\n3.Piercing Blood (+50 Damage,-25 Health Points)";
            cout << "\n\nYour Technique: ";
            cin >> wish;
